...
func (q *ColaCircular) ObtenerMensajesDesdeId(idSala uuid.UUID, idMensaje string) ([] Mensaje, error) {
	q.Mu.Lock()
	defer q.Mu.Unlock()
	...
	for i := 0; i < q.Size; i++ {
		mensaje := q.Buffer[(q.Head+i)%q.Capacity]
		if mensaje.Id == idMensaje { // Asumimos que `Mensaje` tiene un campo `ID`
